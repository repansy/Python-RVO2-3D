...
cdef extern from "Vector3.h" namespace "RVO":
    cdef cppclass Vector3:
        Vector3() except +
        Vector3(float x, float y, float z) except +
        float x() const
        float y() const
        float z() const
...
cdef extern from "RVOSimulator.h" namespace "RVO":
    cdef cppclass RVOSimulator:
        ...
        size_t addAgent(const Vector3 & position, float neighborDist,
                        size_t maxNeighbors, float timeHorizon, float radius, float maxSpeed,
                        const Vector3 & velocity)
        void doStep() nogil
...
cdef class PyRVOSimulator:
    ...
    def getAgentNumORCAPlanes(self, size_t agent_no):
        return self.thisptr.getAgentNumORCAPlanes(agent_no)
    def getAgentORCAPlane(self, size_t agent_no, size_t plane_no):
        cdef Plane plane = self.thisptr.getAgentORCAPlane(agent_no, plane_no)
